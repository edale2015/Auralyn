...
router.post("/build", requireRole(["admin", "physician"]), (req, res) => {
  ...
  const bundle = buildEncounterBundle(parsed.data);
  res.json(bundle);
});
...
router.post("/build-batch", requireRole(["admin"]), (req, res) => {
  const parsed = batchEncounterSchema.safeParse(req.body);
  if (!parsed.success) {
    return res.status(400).json({ error: parsed.error.issues[0].message, details: parsed.error.issues });
  }
  const bundles = parsed.data.encounters.map(buildEncounterBundle);
