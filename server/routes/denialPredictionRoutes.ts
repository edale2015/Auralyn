...
router.post("/predict", requireRole(["admin", "physician"]), async (req, res) => {
...
router.post("/predict-batch", requireRole(["admin"]), async (req, res) => {
  ...
  const result = await batchPredictDenials(inputs);
  res.json(result);
});
