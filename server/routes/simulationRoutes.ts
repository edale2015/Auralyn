...
import { runSimulation, summarizeSimulation, runScenarios } from "../services/simulation/simulationEngine";
...
router.get("/scenarios", async (req, res) => {
  try {
    const n        = Math.min(Number(req.query.n ?? 500), 10_000);
    const scenarios = await runScenarios(n);
    res.json({ ok: true, scenarios });
  } catch (err: any) {
    res.status(500).json({ error: "Scenario simulation failed", detail: err?.message });
  }
});
