...
import { formatTraceForWhatsApp, formatScenarioList } from "../traces/traceSummary";
...
export function isStaffCommand(msg: string): boolean {
  ...
  return lower.startsWith("!scenario") || lower.startsWith("!trace") || lower.startsWith("!case");
}
...
export async function handleStaffCommand(msg: string): Promise<string> {
  ...
  try {
    ...
    return `Unknown command: ${cmd}\n\nAvailable: !scenario list|run <id>, !trace last|<runId>, !case <caseId>`;
  } catch (err: any) {
...
async function handleScenarioCommand(args: string[]): Promise<string> {
  ...
  if (sub === "run") {
    ...
    let result = formatTraceForWhatsApp(stored);
  ...
  return `Unknown subcommand: ${sub}\n\nUsage: !scenario list | !scenario run <id>`;
}
...
async function handleTraceCommand(args: string[]): Promise<string> {
  ...
  if (!sub || sub === "last") {
    ...
    return formatTraceForWhatsApp(trace);
  }
