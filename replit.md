...
- **Decision Tree Explorer** (`/decision-tree`): Full interactive ReactFlow visualization of the clinical reasoning map. Pulls live data from `loadComplaintConfig` — 78 complaints, 63 nodes/62 edges for ent_sore_throat (19 questions, 2 red flags, 21 rules, 6 dispositions, 14 DX candidates). Color-coded by node type (stage/question/redflag/score_rule/disposition/dx). Node click → detail inspect + "Test Node" + "Suggest Fix". Failure path highlighting in red. Golden failures sidebar with per-case path highlight. AI fix suggestions via `POST /api/learning/suggest-fix` (GPT-4o-mini + heuristic fallback). Backend: `server/routes/decisionTreeRoutes.ts`. Frontend: `client/src/pages/DecisionTreePage.tsx`.
