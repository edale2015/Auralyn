...
**B4-5 DERM Rash**: Added 6 optional questions (Q_DRSH_MENINGITIS_SYMPTOMS, Q_DRSH_RAPID_SPREAD, Q_DRSH_HYPOTENSION_LIKE, Q_DRSH_ARTHRALGIA, Q_DRSH_TICK_EXPOSURE, Q_DRSH_FACE_SWELLING) and 3 new clusters (CL_DRSH_SJS_TEN, CL_DRSH_MENINGOCOCCEMIA, CL_DRSH_TICKBORNE). Replaced CL_DRSH_ALLERGIC_DRUG with CL_DRSH_DRUG_EXANTHEM. Enhanced CL_DRSH_VASCULITIC with arthralgia boost and meningitis/SJS suppressors. Replaced CL_DRSH_VIRAL_EXANTHEM rules with clean boosters/suppressors. Added MEN_NEG_NO_FEVER (-3) to prevent meningococcemia without fever. Updated RF_DRSH_SJS_TEN to fire on mucosal+blister (removed new_med requirement). Added RF_DRSH_MENINGITIS for fever+meningitis signs. Updated DISP_DRSH_EMERG to include mucosal+blister and meningitis+fever conditions. DX_PRIORITY for 6 clusters (meningococcemia=100, sjs_ten=95, vasculitic=80, tickborne=70, drug_exanthem=60, viral_exanthem=50). 16 existing goldens updated with new cluster names. 13 hard subtype goldens + 23 existing = 36 total rash tests passing.

Currently 72+ complaints on GENERIC_V1 with 1312 golden tests passing.
