...
- **Operations Dashboard**: Dashboard at `/operations-dashboard` with 6 tabs. Multi-Physician Routing Engine (6 clinicians across 2 clinics, load-balanced scoring with hard high-risk enforcement), Escalation + SLA Queue (10 seeded cases with priority scoring and SLA thresholds: HIGH=5min, MEDIUM=20min, LOW=60min), Outcome Drift Monitor (deterministic baseline/recent windows, moderate drift detected at -17.5%), Per-Complaint Analytics (7 complaint types with accuracy/escalation tracking), Approval Rules Engine (4-tier: auto_approve/eligible_for_batch/mandatory_review/escalate), Immutable SHA-256 Audit Chain (5 seeded entries with integrity verification). Services in `server/services/{physicianRouter,caseQueue,opsMetrics,driftMonitor,approvalRules,complaintAnalytics,auditChain}.ts`. Routes in `server/routes/operationsRoutes.ts`.
