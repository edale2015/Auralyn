...
The **Executive Dashboard** (`/executive-dashboard`) is a 7-tab board-level analytics dashboard with: **Trends** (throughput, override rate, margin, satisfaction charts via Recharts), **Complaint Drilldown** (status-coded complaint performance), **Physician Drilldown** (provider performance metrics), **Cross-Clinic Comparison** (multi-clinic benchmarking with status indicators), **Alert Center** (create/view alerts by severity and type), **Weekly Email Preview** (executive summary email generation), and **CSV Export** (downloadable data exports). Backend services: executiveStore (in-memory persistence for snapshots, simulation runs, control runs), executiveDrilldown, executiveCharts, executiveFilters, exportCsv, crossClinicComparison, alertCenter, weeklyExecutiveMailer. All routes at `/api/executive-db/*` with role-based auth.
