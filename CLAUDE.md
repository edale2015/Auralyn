...
No dedicated lint script exists in `package.json`; treat `npm run check` as the baseline. Playwright is installed, but no Playwright config file was confirmed present; run `npx playwright test` only after verifying config and specs exist. The sanitized export may not type-check because of redaction placeholders.
...
- Root `migrations/` may not exist even though Drizzle config points there. Verify the actual migration pipeline before adding files.
...
- `server/routes.auth.ts` calls `setProviderSession(res)` while `server/auth.ts` defines `setProviderSession(res, user)` — signature mismatch; verify before relying on `/api/auth/login`.
- `server/routes.ts` and `server/routes.auth.ts` both register `/api/auth/login`; the earlier mount wins. Do not add new behavior to the legacy auth path.
...
Never print or commit values. All secrets belong in Replit Secrets (encrypted), never in `.replit` `[userenv.shared]` which is git-tracked plaintext. Known names include: `DATABASE_URL`, `APP_JWT_SECRET`, `JWT_SECRET`, `AUTH_JWT_SECRET`, `SESSION_SECRET`, `CSRF_COOKIE_NAME`, `AUTH_COOKIE_NAME`, `ALLOW_BEARER_AUTH_FALLBACK`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_WHATSAPP_NUMBER`, `TWILIO_WHATSAPP_FROM`, `HARDENING_WEBHOOK_SECRET`, `REDIS_URL`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `FIREBASE_PROJECT_ID`, `FIREBASE_STORAGE_BUCKET`, `SHEETS_SPREADSHEET_ID`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AUDIT_HMAC_SECRET`, `MD_PASSWORD`, `CLINICIAN_PASSWORD`, `CLINIC_PASSWORD`, `DEMO_PASSWORD_*`, `PROVIDER_API_KEY`, `TEST_EXEC_TOKEN`, `AURALYN_WS_ALLOW_VITALS`, `AUDIT_MEMORY_CACHE_SIZE`, `AUDIT_ADVISORY_LOCK_ID`, and integration-specific EHR/FHIR secrets.
