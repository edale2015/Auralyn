 ...
 * System orchestrates across all nine subsystems.
 *
  ...
  return (
    ...
    >
      <div className="shrink-0 mt-0.5">
        {action.status === "complete" && <CheckCircle2 className="h-4 w-4 text-emerald-400" />}
        {action.status === "running"  && <Loader2 className="h-4 w-4 text-blue-400 animate-spin" />}
        {action.status === "pending"  && <div className="h-4 w-4 rounded-full border border-gray-600" />}
      ...
      <div className="flex-1 min-w-0">
        ...
        {action.result && <p className="text-xs text-gray-500 mt-0.5">{action.result}</p>}
        {action.error  && <p className="text-xs text-red-400 mt-0.5">{action.error}</p>}
      </div>
...
export function AuralynCommandInterface({ physicianId }: { physicianId?: string }) {
  ...
  const commandMutation = useMutation({
    mutationFn: (commandText: string) =>
      apiRequest("POST", "/api/command", {
        command:     commandText,
        physicianId: physicianId ?? "phys1",
        confirmed:   showConfirm,
      }).then(r => r.json()) as Promise<CommandResponse>,
      ...
      >
        ...
        <div className="px-4 py-3 border-b border-gray-800/50">
          <textarea
            ...
            placeholder="What do you need? e.g. 'Approve the UTI case' or 'Show escalated follow-ups'"
            ...
            <div className="space-y-4">
              ...
              {!response.error && (
                <button
                  ...
                  data-testid="btn-another-command"
                >
                  + Another command
                </button>
              )}
