...
async function simulateConversation(scenario: typeof TEST_SCENARIOS[0]): Promise<ScenarioResult> {
  ...
  for (let i = 0; i < scenario.conversation.length; i++) {
    ...
    if (erTriggered && !scenario.isSafety && i < scenario.conversation.length - 1) {
      ...
      break; // Stop — Auralyn already ended the conversation
    }
...
/*
ADD TO package.json:
"test:conversation": "npx tsx server/tests/conversationTestHarness.ts",
"test:conversation:watch": "nodemon --exec 'npx tsx server/tests/conversationTestHarness.ts' --watch server/whatsapp --watch server/clinical"
*/
