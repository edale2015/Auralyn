/**
 * ClinicalContextManager — the "harness"
 *
 * This is the deterministic shell around the stochastic model. Every prompt
 * to every agent in the Auralyn pipeline is assembled HERE, not in the agents
 * themselves. That way, when something goes wrong, the failure is in one
 * place — not scattered across six agents.
 *
 * Responsibilities:
 ...
 *   4. Answer queries like "have we already ruled this out?" so agents don't
 *      re-litigate decisions.
 *   5. Record artifacts produced by agents.
 *
 * It does NOT call the model. It does NOT decide what tools to use beyond
 * which ones to expose. Those are upstream concerns. This module is the
 * single source of truth for "what does this agent see right now."
 */
...
// degrades well before the limit. These are the "effective window" sizes.
...
export class ClinicalContextManager {
   ...
   * re-running the same exclusion reasoning over and over.
   */
  isRuledOut(diagnosis: string): { ruledOut: boolean; reason?: string } {
    const match = this.ctx.artifacts.find(
      (a) =>
        ...
        a.payload.diagnosis.toLowerCase() === diagnosis.toLowerCase(),
    );
    if (!match) return { ruledOut: false };
    return {
      ruledOut: true,
      reason: "reason" in match.payload ? match.payload.reason : undefined,
    };
  }
  ...
  hasFailedAttempt(attemptKey: string): boolean {
    return this.ctx.artifacts.some(
      (a) =>
        ...
        a.payload.attempted === attemptKey,
    );
  }
  ...
  addRedFlag(flag: RedFlag): void {
    // Idempotent: never add the same flag twice.
    if (this.ctx.immutables.redFlagsIdentified.some((f) => f.id === flag.id)) {
      return;
    }
  ...
  recordArtifact(a: Artifact): void {
    // Idempotency by id — agents may try to re-record the same finding.
  ...
  /**
   * Build a prompt for a specific agent role. This is the core of the
   * harness: each role gets ONLY what it needs, in a stable order that
   * favors recall (immutables at the top AND at the bottom, working
  ...
  assemblePromptFor(role: AgentRole, instruction: string): AssembledPrompt {
    ...
    // Important: immutables appear TWICE — at the top AND just before the
    // instruction. Middle-of-context recall is poor, so safety-critical
    // facts get bookended.
    const userPrompt = [
    ...
    return {
      ...
      toolNames: [], // populated by the caller via RoleScopedToolRegistry
      excluded: {
  ...
  private serializeWorkingForRole(role: AgentRole): string {
    ...
    // Differential is relevant to almost everyone except billing
    ...
    // Answered questions matter for differential and disposition
    ...
    // Pending dispositions matter for disposition and supervisor
   ...
   * Priority order: red-flag-relevant > role-relevant > recency.
   */
  private selectArtifactsForRole(
    ...
    const candidates = this.ctx.artifacts
      ...
      .sort((a, b) => {
        // failed_attempt and ruled_out are always high priority — they
        // prevent re-litigation. Then recency.
  ...
  private serializeArtifact(a: Artifact): string {
    ...
    const cite =
      a.provenance.citation || (a.provenance.kbChunkIds || []).join(",") || a.provenance.source;
