 ...
 * Drop into: client/src/components/PatientContextPanel.tsx
 *
 ...
 *
 * Usage in CaseReview.tsx — add between Case Summary and Answers cards:
 *
 *   {c.answers?.structured?._ehr_prepopulated && (
 *     <PatientContextPanel
 *       caseId={c.caseId}
 ...
 *   )}
 *
 * Or for manual fetch (physician triggers it):
 *   <PatientContextPanel
 *     caseId={c.caseId}
...
interface PatientContextPanelProps {
  ...
  patientId?:     string;
  vendor?:        string;
  manualTrigger?: boolean;   // if true, show "Load EHR Context" button instead of auto-load
}
...
function severityBadge(severity?: string) {
  ...
  return <Badge variant="outline" className={`text-[10px] px-1.5 py-0 ${cls}`}>{severity ?? "unknown"}</Badge>;
}
...
export function PatientContextPanel({
  ...
  const fetchMutation = useMutation({
    mutationFn: () =>
      apiRequest<{ ok: boolean; context: PatientContext }>(
        "GET",
        `/api/fhir/context/${patientId ?? "demo"}?vendor=${vendor}`
      ),
    onSuccess: (data) => {
  ...
  const vendorLabel = vendor === "mock" ? "Demo EHR" :
      ...
      <CardHeader className="pb-2 pt-3">
        ...
        {!isExpanded && !context && (
          <p className="text-xs text-teal-500 mt-1">
            {manualTrigger
              ? "Click to load patient's EHR record — medications, allergies, conditions, labs"
        ...
        <CardContent className="pt-0 pb-4 space-y-4">
          ...
          {context && !fetchMutation.isPending && (
            <>
              ...
              {/* Allergies — always show first after demographics */}
