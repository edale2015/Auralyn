 ...
 * Drop into: server/infra/selfHealingMonitor.ts
 *
 * THE STACK THAT LIVES — Auralyn's Self-Healing Infrastructure Layer
 *
 * FROM THE ARTICLE:
 * "The monitor caught the timeout. A trigger I'd wired up months earlier
 * fired off, opened a Claude Code session with the failing service's logs
 * as context, and the agent took it from there."
 *
 * FOR AURALYN:
 ...
 *   4. Notifies the admin via the audit chain + console alert
 *   5. Escalates to physician admin if remediation fails
 *
 * THE SIX CRITICAL SERVICES:
 *   1. PostgreSQL connection pool — if exhausted, cases can't be saved
 *   2. BullMQ follow-up worker — if dead, patients miss scheduled messages
 *   3. WebSocket multimodal gateway — if down, ambient notes don't work
 *   4. Drift canary scheduler — if missed, model drift goes undetected
 *   5. Research radar scheduler — if missed, Rec 5/6 monitoring lapses
 *   6. Skill nudge scheduler — if missed, learning loop stalls
 *
 * SAFE REMEDIATION ACTIONS (auto-apply without human):
 *   - Reconnect Postgres pool (safe, non-destructive)
 *   - Re-register BullMQ workers (safe, idempotent)
 *   - Re-arm schedulers (safe, idempotent)
 *   - Clear stuck job locks (safe, recoverable)
 *
 ...
 *
 * REGISTRATION:
 *   import { SelfHealingMonitor } from "./infra/selfHealingMonitor";
 *   SelfHealingMonitor.start();  // call from server/index.ts
 */
...
// ─── Service health state (in-memory, survives per-process) ──────────────────

const healthState = new Map<ServiceName, ServiceHealth>();
const schedulerHeartbeats = new Map<string, number>();  // scheduler → last fire timestamp

// ─── Heartbeat registration ───────────────────────────────────────────────────
// Call this from each scheduler when it fires.
// Enables dead-man's switch detection.
...
async function checkBullMQWorker(): Promise<ServiceHealth> {
  const now = new Date().toISOString();
  try {
    // Check if follow-up jobs have been processed recently
    ...
    // Check for stuck/failed jobs in BullMQ (via Redis if available)
    // Fallback: check if BullMQ queue factory is reachable
...
function checkScheduler(
  ...
  return {
    ...
    details:     lastBeat
      ...
      : "No heartbeat recorded since server start",
  };
}
...
async function checkWebSocketGateway(): Promise<ServiceHealth> {
  const now = new Date().toISOString();
  try {
    // Check if WebSocket gateway has had recent connections
    ...
    // If no cases processed in 4 hours during business hours, flag as degraded
    const hour = new Date().getUTCHours();
    const isBusinessHours = hour >= 12 && hour <= 24; // 8am-8pm EST
...
// The article: "Read the logs. Spotted a stuck-but-not-crashed container."
// For Auralyn: Claude analyzes recent audit events to diagnose the failure.

async function diagnoseFailure(health: ServiceHealth): Promise<string> {
  // Gather recent relevant audit events
...
async function remediatePostgres(health: ServiceHealth): Promise<RemediationResult> {
  try {
    // Attempt to reconnect by running a test query
...
async function remediateBullMQ(health: ServiceHealth): Promise<RemediationResult> {
  try {
    // Re-register the follow-up worker (idempotent)
...
export const SelfHealingMonitor = {
  ...
  // Registry of scheduler re-arm functions (populated at startup)
  ...
  async runHealthChecks(): Promise<ServiceHealth[]> {
    const checks = await Promise.all([
      ...
      Promise.resolve(checkScheduler("research_radar_scheduler",  "research_radar",  170)),  // weekly ≈ 168h
  ...
  async handleFailure(health: ServiceHealth): Promise<IncidentReport> {
    ...
    // Audit event (always fires, even if remediation fails)
    await appendAuditEvent({
      ...
      details: {
        ...
        requiresHuman:   remediation.requiresHuman,
        diagnosisSummary: diagnosis.slice(0, 200),
      },
    }).catch(console.error);

    // Console alert (the "resolution email" equivalent)
  ...
  async runCycle(): Promise<void> {
    ...
    for (const health of healths) {
      const isNewFailure =
        ...
        health.failureCount === 1;  // only alert on first failure, not every cycle

      const isPersistent =
        (health.status === "down") &&
        health.failureCount > 0 &&
        health.failureCount % 6 === 0;  // re-alert every 30 minutes if still down
  ...
  start(): void {
    ...
    // Initial check after 30 seconds (let services start up)
    setTimeout(() => {
