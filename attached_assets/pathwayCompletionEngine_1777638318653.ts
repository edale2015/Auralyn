 ...
 * Drop into: server/clinical/pathwayCompletionEngine.ts
 *
...
export interface CompletionRequest {
  ...
  guidelineSources?: string[];  // specific guidelines to reference
  priorityFields?:  Array<keyof ComplaintPathway>;  // which fields to fill first
}
...
export interface CompletionDraft {
  ...
  requiresPhysicianReview: string[];  // specific items needing physician attention
...
async function draftLRTables(
  ...
  const result = await llmGateway.complete({
    purpose:  "kb_validator",  // Opus — clinical accuracy matters
    messages: [{
      role:    "user",
...
async function draftPhysicalExam(
  ...
  const result = await llmGateway.complete({
    purpose:  "retrieval_pruner",  // Sonnet sufficient for exam drafting
    messages: [{
      role:    "user",
...
async function draftPatientCommunication(
  ...
  const result = await llmGateway.complete({
    purpose:  "discharge_generator",  // Sonnet — patient communication
    messages: [{
      role:    "user",
...
async function draftDriftCanary(
  pathway: Partial<ComplaintPathway>
): Promise<{ id: string; complaint: string; symptoms: string[]; expectedDisposition: string; expectedTopDiagnosis: string; confidenceFloor: number; mustHaveRedFlag: boolean }> {
  // Generate a canonical test case for the drift canary
...
// ─── Batch completion ─────────────────────────────────────────────────────────
// Processes all P1 critical pathways from the master map.
// Runs as a background job — not blocking.
...
export async function runBatchCompletion(
  ...
  for (const slug of slugs) {
    try {
      // Load the partial pathway from the pathway registry or DB
