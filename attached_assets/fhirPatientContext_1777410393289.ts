 ...
 * Drop into: server/integrations/ehr/fhirPatientContext.ts
 *
 * Part 1 of 3: Patient Record Context Layer
 *
 ...
 * Epic (stub, token passthrough), or mock data.
 *
 * PatientContext feeds into:
 *   - Intake pre-population (Part 2)
 *   - CDS sidebar enrichment (Win 2)
 *   - Prior auth skeleton (Part 3)
 *
 * Usage:
 *   const ctx = await fetchPatientContext({ vendor: "ecw", patientId, accessToken });
 *   // ctx.medications, ctx.allergies, ctx.conditions, ctx.demographics, ctx.labs
 */
...
export interface PatientContext {
  ...
  patientId:        string;
  fetchedAt:        string;
  partial:          boolean;    // true if some sections failed
  errors:           string[];   // non-fatal fetch errors per section
  ...
  // Pre-computed intake patch
  // These fields map directly onto answers.structured for pre-population
  intakePatch: {
    name?:        string;
    dob?:         string;
    age?:         number;
    sex?:         string;
    medications:  string[];   // string[] for compatibility with intake field
    allergies:    string[];
    conditions:   string[];
  };
}
...
export interface FetchContextOptions {
  vendor:       EhrVendor;
  patientId:    string;
  accessToken?: string;   // required for ecw/epic; optional for athena (uses env token)
  practiceId?:  string;   // required for athena if not set via env
  fhirBase?:    string;   // required for ecw/epic if not set via env
}

// ─── Normalizers ──────────────────────────────────────────────────────────────
// Convert vendor-specific shapes to unified PatientContext fields

function normalizeMedications(raw: any[], vendor: EhrVendor): Medication[] {
  if (!Array.isArray(raw)) return [];

  return raw.map(item => {
    // FHIR R4 MedicationRequest / MedicationStatement
...
async function fetchFromAthena(options: FetchContextOptions): Promise<Partial<PatientContext>> {
  ...
  // Fetch all sections in parallel — section failure is non-fatal
  ...
  const demographics = normalizeDemographics(patient, "athena");
  const medications  = normalizeMedications(meds?.medications ?? [], "athena");
  const allergyList  = normalizeAllergies(allergies?.allergies ?? [], "athena");
  const conditions   = normalizeConditions(problems?.problems ?? [], "athena");

  return { demographics, medications, allergies: allergyList, conditions, errors };
}
...
async function fetchFromFhirR4(
  ...
  // Normalize labs from Observation resources
...
export async function fetchPatientContext(
  ...
  // Build intake patch — maps PatientContext → answers.structured compatible fields
  const intakePatch = {
