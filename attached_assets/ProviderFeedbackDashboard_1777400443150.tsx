 ...
 * Drop into: client/src/pages/ProviderFeedbackDashboard.tsx
 *
 ...
 *
 * Data sources:
 *   GET /api/provider/feedback              → benchmark grade + personal stats
 *   GET /api/provider/feedback/activity     → audit chain events for this physician
 *   GET /api/provider/feedback/trend        → 30-day volume + approval rate series
 *
 * Route: add to your router as /physician-feedback (or /provider-feedback)
 * Link: add to the clinical nav alongside /physician-dashboard
 */
...
interface OutlierFlag {
  metric:        string;
  deviation:     number;   // % deviation from own 30-day baseline
...
// ─── Mini sparkline (CSS-only, no chart library needed) ───────────────────────

function Sparkline({ points }: { points: TrendPoint[] }) {
  if (!points.length) return null;

  const maxVol = Math.max(...points.map(p => p.caseVolume), 1);
  const barWidth = `${Math.floor(100 / points.length)}%`;
...
export default function ProviderFeedbackDashboard() {
  ...
  const summaryQuery = useQuery({
    ...
    queryFn:   () => apiRequest<FeedbackSummary>("GET", "/api/provider/feedback"),
    staleTime: 60_000,
  });

  const activityQuery = useQuery({
    queryKey:  ["/api/provider/feedback/activity"],
    queryFn:   () => apiRequest<{ events: ActivityEvent[] }>("GET", "/api/provider/feedback/activity"),
    staleTime: 30_000,
  });

  const trendQuery = useQuery({
    queryKey:  ["/api/provider/feedback/trend"],
    queryFn:   () => apiRequest<{ points: TrendPoint[] }>("GET", "/api/provider/feedback/trend"),
    staleTime: 60_000,
  });
          ...
          <>
            ...
            >
              <div className="flex items-start gap-4">
                {/* Grade badge */}
                <div className={`${gradeColor(summary.grade)} text-white rounded-xl w-16 h-16 flex items-center justify-center shrink-0`}>
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-gray-600">
                    <span>{summary.totalCases} cases reviewed</span>
                    <span>·</span>
                    <span>{Math.round(summary.approvalRate * 100)}% approval rate</span>
                    <span>·</span>
                    <span>{Math.round(summary.overrideRate * 100)}% AI override rate</span>
                    <span>·</span>
                    <span>Avg response {formatMs(summary.avgResponseMs)}</span>
                  </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-xs font-medium text-gray-900">
                          {metric.unit === "ms"
                            ...
                            : metric.physicianValue.toFixed(2)
                          }
                        </span>
                        <span className="text-[10px] text-gray-400">
                          vs {metric.unit === "ms"
                            ...
                            : metric.nationalValue.toFixed(2)
                          } national
                        </span>
                        <Badge
                          variant="outline"
