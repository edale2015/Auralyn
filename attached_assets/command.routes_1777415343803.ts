/**
 * command.routes.ts
 * Drop into: server/routes/command.routes.ts
 *
 * Architecture 7 orchestration layer.
 * Receives physician natural language commands, parses intent via Claude,
...
// Subsystem imports — all already built in Wins 1-9
...
// ─── Intent parser ────────────────────────────────────────────────────────────
// Uses Claude to parse physician's natural language into structured intent.
...
async function parseIntent(command: string, physicianId: string): Promise<ParsedIntent> {
...
async function executeQueueView(intent: ParsedIntent, physicianId: string) {
...
async function executeFollowupView(intent: ParsedIntent, physicianId: string) {
  ...
  return {
    ...
    summary: escalated.length > 0
      ? `⚠️ ${escalated.length} patient${escalated.length !== 1 ? "s" : ""} need attention. Navigate to /follow-up-monitoring for details.`
      : `All ${active.length} active follow-up patients are responding normally. Navigate to /follow-up-monitoring for details.`,
  };
}

async function executePerformance(intent: ParsedIntent, physicianId: string) {
  // Return a summary — full data is on the dashboard
  return {
    actions: [{
      type:   "PERFORMANCE",
...
async function executeEhrContext(intent: ParsedIntent, physicianId: string) {
...
async function executePriorAuth(intent: ParsedIntent, physicianId: string) {
...
commandRouter.post(
  ...
  async (req, res) => {
    ...
    const actor = physicianId ?? req.user?.id ?? "phys1";

    try {
      // Parse intent
      const intent = await parseIntent(command, actor);
      ...
      switch (intent.category) {
        case "QUEUE_VIEW":      result = await executeQueueView(intent, actor);    break;
        ...
        case "PERFORMANCE":     result = await executePerformance(intent, actor);  break;
        case "EHR_CONTEXT":     result = await executeEhrContext(intent, actor);   break;
        case "PRIOR_AUTH":      result = await executePriorAuth(intent, actor);    break;
        case "TELEMED_VIEW":
          result = {
      ...
      await appendAuditEvent({
        ...
        details: {
          ...
          confirmed,
          // command text intentionally omitted — may contain PHI
        },
      }).catch(() => {});
