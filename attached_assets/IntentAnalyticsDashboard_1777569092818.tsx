 ...
 * Drop into: client/src/pages/IntentAnalyticsDashboard.tsx
 *
 * Architecture 7 analytics — shows which intents physicians actually express
 * vs which ones they try and abandon. This feeds:
 *   1. Command interface prioritization (which intents need better natural language coverage)
 *   2. Clinical Skills loop (what physicians ask about most = what skills to prioritize)
 *   3. DashboardContextPrompt optimization (which pre-filled commands get clicked)
 *
 * Route: /intent-analytics (admin only)
 *
 * The article: "Designing for adaptation means building feedback loops
 * through which the environment learns from each interaction."
 * This dashboard IS that feedback loop, made visible.
 */
...
function IntentBar({ stat }: { stat: IntentStat }) {
  ...
  const barColor   = stat.successRate >= 0.8 ? "bg-green-500" :
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span className={`font-semibold ${successPct >= 80 ? "text-green-600" : successPct >= 50 ? "text-yellow-600" : "text-red-600"}`}>
            {successPct}%
          </span>
        ...
        <Card>
          ...
          <CardContent className="pb-4 space-y-3">
            {isLoading ? (
              ...
              <p className="text-xs text-gray-500">No command analytics yet. Start using ⌘K and check back.</p>
            )}
          </CardContent>
        </Card>

        {/* Low success categories */}
        {data?.lowSuccessCategories?.length > 0 && (
          <Card className="border-amber-200">
            <CardHeader className="pb-2 pt-4">
              ...
              <p className="text-xs text-amber-600 mt-0.5">
                ...
                Add better natural language coverage or surface them as DashboardContextPrompt suggestions.
              </p>
            </CardHeader>
            <CardContent className="pb-4 space-y-3">
          ...
          <Card className="border-gray-200">
            <CardHeader className="pb-2 pt-4">
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                ...
                Consider adding them to DashboardContextPrompt suggestions or onboarding.
              </p>
            </CardHeader>
            <CardContent className="pb-3 flex flex-wrap gap-1.5">
              {data.unusedCategories.map(cat => (
                <Badge key={cat} variant="outline" className="text-[10px] text-gray-400">
