 ...
 * Drop into: server/retrieval/clinicalDocumentChunker.ts
 *
...
export function structuralSplit(
  ...
  function getPage(lineIndex: number): number {
    // Approximate page from character position
  ...
  function flushSection(): void {
    ...
    if (text.length > 50) {  // minimum meaningful chunk
      sections.push({
        text,
...
export function mergeIncompleteChunks(sections: RawSection[]): RawSection[] {
  ...
  for (const section of sections) {
    ...
    // Merge if pending ends mid-sentence or mid-drug-protocol
...
function extractClinicalTerms(text: string): string[] {
  ...
  terms.push(...drugs.map(d => d.toLowerCase()));
  ...
  // Conditions — ICD-adjacent terms
  const conditionPattern = /\b([A-Z][a-z]+ (?:syndrome|disease|failure|injury|infection|disorder|pneumonia|sepsis|embolism))\b/g;
  const conditions = text.match(conditionPattern) ?? [];
  terms.push(...conditions.map(c => c.toLowerCase()));
...
function scoreRetrievalWeight(chunk: Omit<ClinicalChunk, "retrievalWeight">): number {
  ...
  // Recommendations and drug protocols are highest value for retrieval
  if (chunk.chunkType === "recommendation") score += 0.3;
  if (chunk.chunkType === "drug_protocol")  score += 0.25;
  if (chunk.chunkType === "diagnostic_criteria") score += 0.2;
  if (chunk.chunkType === "algorithm")      score += 0.15;

  // Background and evidence tables are lower retrieval value
  if (chunk.chunkType === "background")     score -= 0.2;
  if (chunk.chunkType === "evidence_summary") score -= 0.1;

  // Evidence level boosts retrieval weight
  if (chunk.evidenceLevel) score += 0.1;

  // Clinical terms density boosts weight
  score += Math.min(chunk.clinicalTerms.length * 0.02, 0.1);
...
async function checkSemanticCoherence(chunk: RawSection): Promise<{
  isCoherent:      boolean;
  suggestedSplits: number[];  // character positions to split at
  reason:          string;
}> {

  // Fast heuristic: if chunk is within reasonable size and has a clear heading, it's coherent
  const wordCount = chunk.text.split(/\s+/).length;
  if (wordCount <= 200 && chunk.heading) {
    return { isCoherent: true, suggestedSplits: [], reason: "Within size limit with clear heading" };
  }

  // Only call LLM for large or ambiguous chunks
  if (wordCount > MAX_CHUNK_WORDS) {
    const result = await llmGateway.complete({
      purpose:  "retrieval_pruner",  // Sonnet — appropriate for this classification task
      messages: [{
        role:    "user",
...
export async function chunkClinicalDocument(params: {
  ...
  // Step 3 (optional): Semantic coherence check for large chunks
        ...
        if (!coherence.isCoherent && coherence.suggestedSplits.length > 0) {
          ...
          for (const splitPos of coherence.suggestedSplits) {
            if (splitPos > lastSplit && splitPos < words.length) {
              const text = words.slice(lastSplit, splitPos).join(" ");
              finalSections.push({
                ...section,
                text,
  ...
  return finalSections.map((section, index) => {
    ...
    return {
      ...partial,
      retrievalWeight: scoreRetrievalWeight(partial),
    };
  });
}

// ─── Retrieval-weighted query ─────────────────────────────────────────────────
...
export function rankChunksByRelevance(
  ...
  return chunks.map(chunk => {
    ...
    // Boost if query terms appear in clinical terms
    const termOverlap = chunk.clinicalTerms.filter(t =>
      queryLower.includes(t.toLowerCase())
    ).length;
    score += termOverlap * 0.1;

    // Boost if query mentions evidence level
    if (queryLower.includes("evidence") || queryLower.includes("guideline")) {
      if (chunk.evidenceLevel) score += 0.15;
    }

    // Boost if query is about treatment and chunk is drug protocol
    if (/treat|medic|drug|dose|prescri/i.test(query) && chunk.chunkType === "drug_protocol") {
      score += 0.2;
    }

    // Boost if query is diagnostic and chunk is diagnostic criteria
...
// ─── Integration with clinicalDocumentIndexer.ts ─────────────────────────────
/*
 * In server/retrieval/clinicalDocumentIndexer.ts, find the section
 * that splits the extracted PDF text into pages/chunks.
 *
 * BEFORE (page-based):
