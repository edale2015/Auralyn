 ...
 * Drop into: client/src/components/AmbientNotePanel.tsx
 *
 ...
 *   />
 *
 * Add to lucide-react imports in CaseReview.tsx:
 *   Mic, MicOff   (not yet imported per line 9 audit)
 */
...
interface AmbientNotePanelProps {
  ...
  complaint?: string;
  /** Called with each transcribed phrase — parent appends to notes textarea */
  onTranscript: (text: string) => void;
  /** Called with full SOAP rawText on stamp — parent replaces/appends notes */
...
// ─── WebSocket singleton ──────────────────────────────────────────────────────
// One socket per panel instance, created on mic-start, closed on unmount.
...
export function AmbientNotePanel({
  ...
  const startListening = useCallback(() => {
    ...
    recognition.interimResults = false; // only send final phrases to socket
    ...
    recognition.onresult = (e: SpeechRecognitionEvent) => {
      for (let i = e.resultIndex; i < e.results.length; i++) {
        if (e.results[i].isFinal) {
          ...
          // Also surface immediately in notes (before socket round-trip)
          onTranscript(phrase);
