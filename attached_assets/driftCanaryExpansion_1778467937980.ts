 ...
 * Drop into: server/harness/driftCanaryExpansion.ts
 *
...
export const EXPANDED_CANARIES: CanaryCase[] = [
  ...
  // P5: Anticoagulated patient + trauma — escalate sooner
  {
    ...
    expectedDisposition:  "er_send",   // escalate due to anticoagulation
  ...
  // P3: Diabetic foot without danger signs — appropriate UC management
  {
    ...
    confidenceFloor:      0.60,
    mustNotHaveRedFlag:   true,
    // Note: escalation criteria should still surface in differential
  },
];
...
export function getCoverageReport(): {
  ...
  systemsWithP2:    string[];   // systems with at least one must-escalate canary
  systemsWithP3:    string[];   // systems with at least one must-not-over-escalate canary
  gapSystems:       string[];   // systems with no canary coverage
} {
  const ALL_SYSTEMS = [
  ...
  for (const canary of EXPANDED_CANARIES) {
    // Infer system from complaint slug
...
function inferSystem(complaint: string): string {
  const map: Record<string, string> = {
    chest_pain: "cardiovascular", syncope: "cardiovascular", palpitations: "cardiovascular",
    leg_swelling: "cardiovascular", dvt: "cardiovascular",
    shortness_of_breath: "respiratory", asthma_exacerbation: "respiratory",
    hemoptysis: "respiratory",
    abdominal_pain: "gastrointestinal", nausea_vomiting: "gastrointestinal",
    rectal_bleeding: "gastrointestinal",
    uti: "genitourinary", testicular_pain: "genitourinary", pelvic_pain_female: "genitourinary",
    back_pain: "musculoskeletal", monoarticular_joint: "musculoskeletal",
    skin_infection: "dermatology", rash_mild: "dermatology",
    headache: "neurology", stroke_tia: "neurology", meningitis_screen: "neurology",
    concussion: "neurology",
    sore_throat: "ent", ear_pain: "ent",
    eye_complaint: "ophthalmology", pink_eye: "ophthalmology",
    suicidal_ideation: "psychiatric", anxiety_panic: "psychiatric",
    medication_overdose: "toxicology", joint_pain_polyarticular: "toxicology",
    head_trauma: "trauma",
    vaginal_bleeding: "gynecology",
    pediatric_fever: "pediatric", epiglottitis: "pediatric",
