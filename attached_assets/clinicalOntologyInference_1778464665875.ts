 ...
 * Drop into: server/ontology/clinicalOntologyInference.ts
 *
...
export interface InferenceCondition {
  field:    string;         // patient context field or KB field
...
export interface InferenceAssertion {
  field:    string;         // field to set in derived context
  value:    any;
  confidence: number;       // 0.0-1.0 confidence in this derived fact
}
...
export interface InferenceRule {
  ...
  name:        string;
  description: string;
  conditions:  InferenceCondition[];   // ALL must be true (AND logic)
  assertions:  InferenceAssertion[];   // All are derived when conditions met
  priority:    number;                 // higher = evaluated first
  clinicalBasis: string;              // guideline or evidence source
  active:      boolean;
}

// ─── Clinical inference rules ─────────────────────────────────────────────────
// These derive high-level clinical facts from raw patient data.
// Each rule is explicit, auditable, and traceable to a guideline.

export const CLINICAL_INFERENCE_RULES: InferenceRule[] = [

  // ── High-risk modifier inference ──────────────────────────────────────────
  {
...
export interface InferenceResult {
  derivedFacts:    Record<string, any>;   // all asserted facts
...
export function runInference(
  ...
  for (const rule of sorted) {
    ...
    if (allConditionsMet) {
      ...
      // Apply assertions to derived facts
      for (const assertion of rule.assertions) {
        // Only override if new assertion has higher confidence
...
// Permission rules baked into the ontology.
// These enforce that only the right actor can take the right action
// given the current clinical state.
...
export function checkActionPermission(
  ...
  const relevantConstraints = ACTION_CONSTRAINTS.filter(c => {
    ...
    return c.actionName.toLowerCase().includes(actionName.toLowerCase()) ||
  ...
  // No matching constraint — default allow for non-clinical actions
  return { permitted: true, reason: "No constraint applies" };
}

// ─── Graph traversal retrieval ────────────────────────────────────────────────
// Follow edges from a complaint to retrieve connected KB as a subgraph.
// This replaces 5 separate table queries with one connected traversal.
...
export async function traverseComplaintGraph(
  ...
  // Hop 1: Red flags for this complaint
  ...
  // Hop 1: Diagnoses for this complaint
  ...
  for (const dx of diagnoses.rows as any[]) {
    ...
    if (!visited.has(nodeId)) {
      ...
      // Hop 2: Treatments for each diagnosis
      if (maxHops >= 2) {
...
// Converts the traversed subgraph into a structured prompt block.
// Superior to flat KB retrieval: relationships are explicit.
