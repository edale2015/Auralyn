 ...
 * Drop into: server/learning/clinicalSkillsSystem.ts
 *
 * HERMES-INSPIRED CLINICAL SKILLS SYSTEM
 *
 * THE CORE IDEA FROM THE ARTICLE:
 * "When Hermes completes a task successfully, it doesn't just log the result.
 * It runs a post-execution evaluation, identifies the exact sequence of steps
 * that produced the outcome, then codifies that sequence into a reusable Skill."
 *
 * FOR AURALYN:
 ...
 *   Override → Evaluate → Extract → Codify → Retrieve → Inject → Better triage
 *
 * WHAT A CLINICAL SKILL LOOKS LIKE:
 *   A markdown file describing: when this skill applies, what the AI tends to
 *   get wrong, what the correct clinical reasoning is, and how confident we are.
 *   Generated by Claude from physician override patterns. Reviewed by physician
 *   before activation. Never auto-applied without approval.
 *
 * MEMORY TIERS (Hermes mapping):
 *   Tier 1 → ACTIVE_SKILLS (top 5 relevant skills, injected into system prompt)
 *   Tier 2 → PHYSICIAN_PROFILE (per-physician override patterns, 500 tokens)
 *   Tier 3 → SKILL_ARCHIVE (SQLite full-text search, all historical skills)
 *   Tier 4 → KB rules + ontology (existing Win 14 layer)
 */
...
export interface ClinicalSkill {
  ...
  title:           string;
  trigger:         string;      // when does this skill apply?
  aiTendency:      string;      // what does the AI typically get wrong here?
  correctReasoning: string;     // what should the AI do instead?
  evidenceBasis:   string;      // what drives this correction?
  confidence:      number;      // 0-1, based on override frequency
  overrideCount:   number;      // how many overrides generated this
  activatedAt?:    string;      // when physician approved
...
export interface SkillRetrievalResult {
  ...
  promptInjection: string;      // ready-to-inject Tier 1 memory block
  tokenEstimate:   number;
}

// ─── Skills storage ───────────────────────────────────────────────────────────
// Stored in Postgres for query + in filesystem as markdown for Claude Code access
...
async function persistSkill(skill: ClinicalSkill): Promise<void> {
  ...
  // Write markdown file (for Claude Code / AGENTS.md reference)
  fs.writeFileSync(
    path.join(SKILLS_DIR, `${skill.id}.md`),
    skillToMarkdown(skill)
  );

  // Write to Postgres (for query and retrieval)
...
// ─── Skill generator ──────────────────────────────────────────────────────────
// Runs after the weekly quality review identifies high-override patterns.
// Claude analyzes the override data and generates the skill document.

export async function generateSkillFromOverrides(overrideData: {
  complaintSlug:    string;
  overrideCount:    number;
  overrideRate:     number;
  aiOutputSamples:  string[];   // what the AI said
  physicianActions: string[];   // what physicians changed it to
  timeframe:        string;
}): Promise<ClinicalSkill | null> {

  if (overrideData.overrideCount < 3) return null;  // need minimum signal
...
// Retrieves the most relevant active skills for a given case.
// Returns a prompt injection block for the LLM system prompt.
...
export async function retrieveRelevantSkills(
  ...
  // Query active skills for this complaint (Tier 3 → promote to Tier 1)
  ...
  // Build Tier 1 memory injection block
  // Hermes pattern: cap at ~800 tokens, inject at session start
  const promptInjection = `
## CLINICAL SKILLS — Active Playbooks for ${complaintSlug.replace(/_/g, " ")}
${skills.length > 0 ? "These skills were generated from physician override patterns. Apply them before forming your differential." : ""}
...
// Instead of passively waiting, actively prompts skill extraction during low-activity.
// Run this during off-peak hours (e.g., 3am UTC) to extract skills from recent overrides.
...
export async function runPeriodicSkillNudge(): Promise<{
  ...
  // Find high-override complaint slugs from last 30 days
  ...
  for (const row of overrideRows.rows as any[]) {
    ...
    // Skip if we already have a recent active skill for this complaint
